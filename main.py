...
volume_master = 0.3 #коэффициент громкости для мастер-канала
...
def callback(in_data, frame_count, time_info, status):
    ...
    for key, v in enumerate(keys):
        ...
        if v[0] > 0:
            ...
            d = (np.sin(2*np.pi*np.arange(phase*buffer_size, (phase+1)*buffer_size)*f/fs)*v[0]).astype(np.float32)
            keys[key][1] = phase+1
        ...
        if v[0] == 0 and v[1] > 0:
            ...
            x = int((0.5 - phase*buffer_size*f/fs % 0.5)*fs/f)
            #print(x, (phase*buffer_size+x)*f/fs)
            #если частота низкая и нельзя закончить синусоиду в пределах буфера
            if x+1 > buffer_size:
                d = (np.sin(2*np.pi*np.arange(phase*buffer_size, phase*buffer_size+buffer_size)*f/fs)*v[2]).astype(np.float32)
                keys[key][1] = phase+1
            #заканчиваем в пределах буфера
            else:
                d1 = (np.sin(2*np.pi*np.arange(phase*buffer_size, phase*buffer_size+x+1)*f/fs)*v[2]).astype(np.float32)
